...
export const AppContextProvider = ({ children }) => {
  ...
  const completeOnboarding = useCallback(async (userData) => {
    ...
    try {
      await safeFetch(`${API_BASE_URL}/api/patients`, {
        method: 'POST',
  ...
  const addProfileBooking = async (profileId, newBooking) => {
    ...
    try {
      await safeFetch(`${API_BASE_URL}/api/bookings`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
      ...
      try {
        ...
        const hydratedProfiles = await Promise.all(
          (ownData.profiles || []).map(async (localProfile) => {
            ...
            try {
              const bRes = await safeFetch(`${API_BASE_URL}/api/bookings/${localProfile.id}`);
              if (bRes.ok) {
            ...
            try {
              const dRes = await safeFetch(`${API_BASE_URL}/api/diagnostics/${localProfile.id}`);
              if (dRes.ok) {
